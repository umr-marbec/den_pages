...
# Vers quel logiciels ou techniques ce tourner
...
::: callout-important
Pour les non-initiés, il est important de différencier un service de type drive d'une solution de sauvegarde que nous allons aborder ici. Bien que ce type de service permette de sauvegarder des données en les stockant sur un serveur, par exemple un cloud, cela ne remplace pas une stratégie de sauvegarde complète. Par ailleurs, il faut aussi différencier les stratégies qui vont permettre de faire un miroir de ces données à l'instant t, d'un processus de sauvegarde qui va quelque part créer une archive pour chaque sauvegarde. À tire d'exemple, une copie miroir d'un jour va généralement écraser la copie précédente alors qu'une tache de sauvegarde type BorgBackup va faire cohabiter plusieurs archives correspondant à chaque sauvegarde et les purger/garder en fonction des règles établies par l'utilisateur.

Si on veut même pousser la réflexion un peu plus loin, un processus de sauvegarde complet doit respecter la règle du [**3-2-1**]{style="color:#ca0030;"} : [**3**]{style="color:#ca0030;"} copies de vos données, sur [**2**]{style="color:#ca0030;"} supports différents et [**1**]{style="color:#ca0030;"} copie hors site. Dans notre première proposition a savoir une sauvegarde sur un support externe, on en respecte pas cette règle, car on ne dispose que de deux copies (une en local et l'autre sur le périphérique externe), probablement sur le même site. Avec la seconde proposition, une sauvegarde sur un serveur distant, on respectera cette règle (le serveur sera lui-même sauvegardé sur un autre serveur).
:::

# 1 - Premier processus : sauvegarde vers un support externe

Comme explique précédement, le premier processus que nous allons aborder ici est la réalisation 
